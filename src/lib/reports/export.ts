...
const LEGEND_ROWS: [string, string][] = [
  ...
  [
    'Lotação da agenda',
    'Horários marcados ÷ capacidade do dia (Metas). Pode passar de 100% com overbook.',
  ],
  ...
  ['Vagas hoje', 'Capacidade do dia − horários já marcados.'],
  ['Vagas 2h', 'Estimativa de encaixes nas próximas 2h.'],
...
function capaRows(run: ReportRunDetail): (string | number | null)[][] {
  ...
  if (
    ...
    notes.push(
      'Receita MTD aguardando caixa Avec (nenhum dia do mês com faturamento conhecido ainda).',
    )
  }
