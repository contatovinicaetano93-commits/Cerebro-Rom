...
export async function buildLiveOverview(asOf?: string): Promise<CerebroOverview> {
    ...
    fetchErrors.push({
      ...
      action:
        cfg.meta.slug === 'rom-brasil'
          ...
          : 'Completar NEON_IGUATEMI_DATABASE_URL (pooler Supabase) na Vercel',
    })
  ...
  if (liveUnits.length < 2) {
    nextActions.unshift({
      ...
      action: 'Completar DATABASE_URL (Brasil=Supabase, Iguatemi=Supabase)',
    })
  }
...
export async function buildOverview(asOf?: string): Promise<CerebroOverview> {
  ...
  if (!hasDb) {
    if (isProd) {
      return {
        ...degradedOverview(
          'DATABASE_URL das unidades ausente em produção (Brasil e Iguatemi = Supabase pooler)',
