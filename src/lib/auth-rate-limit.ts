 ...
 * Neon outage resets it to false so the next healthy call re-creates if needed.
 */
...
export async function isLoginBlocked(
  ...
  try {
    ...
    // Neon down — fall back to in-memory so login is never blocked by a DB outage.
    _tableEnsured = false
    return RateLimiter.isBlocked(key, maxRequests, windowSeconds)
  }
}
...
export async function recordLoginFailure(
  ...
  try {
    ...
    // Neon down — fall back to in-memory rate limiter.
    _tableEnsured = false
    const ok = RateLimiter.checkLimit(key, maxRequests, windowSeconds)
    return {
      blocked: !ok,
