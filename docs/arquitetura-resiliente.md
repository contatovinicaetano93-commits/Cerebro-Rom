...
2. **full fatiado** (`/api/avec/sync/full/{ops,agenda,catalog}`): 2×/dia (10h/22h). Retry on-demand respeita `FULL_RETRY_MIN_GAP` (~45min) — **não há cron horário**. **agenda** traz janela +21d (Contatos Agendados +7d). **ops** = P1/P2/P3; **catalog** = 0004.
