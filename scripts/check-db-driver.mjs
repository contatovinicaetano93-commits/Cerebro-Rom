#!/usr/bin/env node
/**
 * Guard against regressing Brasil reads to Neon HTTP.
 * neon() (@neondatabase/serverless) cannot talk to *.supabase.com / pooler hosts —
 * production BR uses Supabase + postgres.js.
 */
...
if (!deps.postgres) {
  errors.push('package.json must depend on `postgres` (postgres.js) for Supabase/Neon TCP')
}
...
// Strip block/line comments before scanning for neon imports / calls.
const codeOnly = dbTs
  .replace(/\/\*[\s\S]*?\*\//g, '')
  .replace(/^\s*\/\/.*$/gm, '')
if (
